...
|                                                       | `paper-universal` | `paper-unrelocated` | `spigot-internals` | `spigot-unrelocated` |
|-------------------------------------------------------|:-----------------:|:-------------------:|:------------------:|:--------------------:|
| Требуемое ядро                                        | Paper **1.18.1+** |  Paper **1.20.6+**  |  Spigot **1.8+**   |   Spigot **26.1+**   |
| Схема реализации                                      |     Paper API     |         NMS         |        NMS         |         NMS          |
| Обновляется после выхода<br>новой версии игры         |        Нет        |         Нет         |         Да         |         Нет          |
| Поддерживает изменение<br>никнейма исполнителя команд |        Нет        |         Да          |         Да         |          Да          |
| Требует наличия сборок<br>Spigot нужных версий        |        Нет        |         Нет         |         Да         |         Нет          |
| Поддерживает Folia                                    |        Да         |         Да          |         Да         |          Да          |
...
> [!NOTE]
> Между `paper-unrelocated`, `spigot-unrelocated` и `spigot-internals` есть тонкая грань.<br>
...
> добавлять поддержку для новых версий игры в `paper-internals`. Аналогичная ситуация произошла
> и с ядром Spigot с версии **26.1** — теперь пакет CraftBukkit тоже без версии
...
- `buildPlatforms` *(по умолчанию — **all**)*<br>
  ...
  Возможные значения: *all*, *paper-universal*, *paper-unrelocated*, *spigot-internals*, *spigot-unrelocated*.<br>
